...
from .permissions import ReadOnly, IsAuthorOrReadOnly, IsAdminOrReadOnly, IsAdminUser
...
#attaches the author of Article
...
def admin_publish_api_endpoint(request, pk, category):
    # use the pk value in the url to target the specific record in db
    try:
        instance = Article.objects.get(pk = pk)
    except Article.DoesNotExist:
        return Response(status=status.HTTP_404_NOT_FOUND)
    instance.category = category
    # use the category value in the url to update the category value for the record
    # change the phase value of the record to 'PUB'
    instance.phase = 'PUB' 
    # save the article
    instance.save()
