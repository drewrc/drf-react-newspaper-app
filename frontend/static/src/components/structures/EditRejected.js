...
function EditRejected({ id, category, title, user_name, img, text }) {
    ...
    const handleSubmit = async (formData) => {
      ...
      const response = await fetch(`/api_v1/articles/rejected/${id}/`, options);
      ...
      <div className='article-container'>
        ...
        <Form onSubmit={handleEdit}>
          <div key={id}>
            ...
            <p>Edit title:</p>
            <textarea id='edit-title-container' className='title' style={{ height: '40px', width: '80%' }} value={editContent.title} onChange={e => setEditContent({ ...editContent, title: e.target.value })} />
            <p>{user_name}</p>
            <div className='text'>
